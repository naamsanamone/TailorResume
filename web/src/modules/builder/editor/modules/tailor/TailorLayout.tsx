...
type Step = 'input' | 'analysis' | 'tailoring';
...
const ScoreBar = ({ score, label, small }: { score: number; label?: string; small?: boolean }) => (
  <div className={small ? 'mb-1' : 'mb-2'}>
    {label && <div className={`flex justify-between ${small ? 'text-xs' : 'text-sm'} mb-0.5`}>
      <span className="text-gray-600">{label}</span>
      <span className="font-semibold" style={{ color: scoreColor(score) }}>{Math.round(score)}%</span>
    </div>}
...
const SkillBadges = ({ skills, type }: { skills: string[]; type: 'matched' | 'missing' | 'partial' }) => {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {skills.map((s, i) => (
        <span key={i} className={`px-2 py-0.5 rounded text-xs ${colors[type]}`}>{s}</span>
      ))}
    </div>
  );
};
...
const TailorLayout = () => {
  ...
  const handleTailorBullets = useCallback(async (index: number) => {
  ...
  const applyAll = useCallback(() => {
    ...
    setApplied(true);
  }, [tailoredSummary, tailoredBullets, tailoredSkills, analysis]);
      ...
      {step === 'analysis' && analysis && (
        <>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={startOver} className="text-xs text-blue-600 hover:underline">← New JD</button>
          </div>
          ...
          <div className="p-4 bg-white rounded-lg shadow-sm border mb-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-3xl font-bold" style={{ color: scoreColor(analysis.overall_score) }}>
          ...
          <div className="space-y-2 mb-4">
            {analysis.matched_skills.length > 0 && (
              <div className="p-2 bg-green-50 border border-green-200 rounded">
                <div className="text-xs font-semibold text-green-800 mb-1">✅ Matched ({analysis.matched_skills.length})</div>
                <SkillBadges skills={analysis.matched_skills.map(s => s.skill)} type="matched" />
              </div>
            )}
            {analysis.missing_skills.length > 0 && (
              <div className="p-2 bg-red-50 border border-red-200 rounded">
                <div className="text-xs font-semibold text-red-700 mb-1">❌ Missing ({analysis.missing_skills.length})</div>
                <SkillBadges skills={analysis.missing_skills} type="missing" />
              </div>
            )}
            {analysis.partial_matches.length > 0 && (
              <div className="p-2 bg-amber-50 border border-amber-200 rounded">
                <div className="text-xs font-semibold text-amber-700 mb-1">⚠️ Partial ({analysis.partial_matches.length})</div>
          ...
          <h3 className="text-sm font-bold text-gray-700 mb-2">Section Scores & Tailor</h3>
          ...
          {analysis.section_scores.summary && (
            <div className="p-3 bg-white border rounded-lg mb-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold" style={{ color: scoreColor(analysis.section_scores.summary.score) }}>
                ...
                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-green-700">
                      {Math.round(tailoredSummary.before_score || 0)}% → {Math.round(tailoredSummary.ats_score)}%
                    </span>
                  </div>
                  <p className="text-gray-600 line-clamp-3">{tailoredSummary.tailored_summary}</p>
                </div>
              ) : (
                <button
                  onClick={handleTailorSummary}
                  disabled={tailoringSection === 'summary'}
                  className="w-full py-1.5 px-3 bg-blue-600 text-white rounded text-xs font-semibold hover:bg-blue-700 disabled:opacity-50"
                >
                  {tailoringSection === 'summary' ? '⏳ Tailoring...' : '✨ Tailor Summary'}
                </button>
              )}
            </div>
          )}
          ...
          {analysis.section_scores.experience && (
            <div className="p-3 bg-white border rounded-lg mb-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold" style={{ color: scoreColor(analysis.section_scores.experience.score) }}>
              ...
              {(analysis.section_scores.experience as any).entries?.map((entry: any, i: number) => (
                <div key={i} className="ml-2 mt-2 p-2 border-l-2 border-gray-200">
                  <div className="flex items-center justify-between mb-1">
                  ...
                  {tailoredBullets[i] ? (
                    <div className="mt-1 p-1.5 bg-green-50 border border-green-200 rounded text-xs">
                      <div className="flex justify-between">
                        <span className="font-semibold text-green-700">✅ Tailored</span>
                        <span className="text-green-700">
                          {Math.round(tailoredBullets[i].before_score || 0)}% → {Math.round(tailoredBullets[i].ats_score)}%
                        </span>
                      </div>
                    </div>
                  ) : (
                    <button
                      ...
                      className="w-full mt-1 py-1 px-2 bg-blue-600 text-white rounded text-xs font-semibold hover:bg-blue-700 disabled:opacity-50"
                    >
                      {tailoringSection === `bullets-${i}` ? '⏳...' : '✨ Tailor Bullets'}
                    </button>
                  )}
                </div>
              ))}
          ...
          {analysis.section_scores.skills && (
            <div className="p-3 bg-white border rounded-lg mb-3">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold" style={{ color: scoreColor(analysis.section_scores.skills.score) }}>
              ...
              {tailoredSkills ? (
                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs">
                  <span className="font-semibold text-green-700">✅ Skills reordered & expanded</span>
                ...
                <button
                  ...
                  className="w-full py-1.5 px-3 bg-blue-600 text-white rounded text-xs font-semibold hover:bg-blue-700 disabled:opacity-50"
                >
                  {tailoringSection === 'skills' ? '⏳ Reordering...' : '✨ Reorder & Expand Skills'}
                </button>
              )}
            </div>
          )}
          ...
          {hasTailoredAnything && (
            <div className="space-y-2 mt-4">
              {!applied ? (
                <button
                  onClick={applyAll}
                  className="w-full py-2.5 px-4 bg-green-600 text-white rounded-md font-semibold text-sm hover:bg-green-700"
                >
                  ✅ Apply All Changes
                </button>
              ) : (
                <>
                  <div className="p-2 bg-green-50 border border-green-200 rounded text-xs text-green-700 text-center">
                    ✅ Changes applied! Resume preview updated.
                  </div>
                  <button
                    onClick={revertAll}
                    className="w-full py-2 px-4 bg-amber-500 text-white rounded-md font-semibold text-sm hover:bg-amber-600"
                  >
