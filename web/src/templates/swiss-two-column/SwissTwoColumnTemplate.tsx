...
const Heading = ({ title, p }: { title: string; p: ResumePalette }) => (
  <h2 style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px', color: p.text, margin: '14px 0 6px', paddingBottom: '3px', borderBottom: `1.5px solid ${p.text}`, fontFamily: p.headingFont }}>{title}</h2>
);
    ...
    <div>
      ...
      {education.map((e) => (
        <div key={e.id} style={{ marginBottom: '6px' }}>
          ...
          {e.score && <div style={{ fontSize: '10px', color: p.muted }}>GPA: {e.score}</div>}
        </div>
      ))}
    </div>
  ) : null;
...
export default function SwissTwoColumnTemplate() {
  ...
  return (
    <div style={{ ...pageStyle(p), padding: 0 }}>
      {/* Header */}
      <div style={{ textAlign: 'center', padding: '28px 40px 16px', borderBottom: `1px solid ${p.divider}` }}>
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: p.text, margin: 0, letterSpacing: '2px', textTransform: 'uppercase', fontFamily: p.headingFont }}>{data.basics.name}</h1>
        <div style={{ fontSize: '10px', color: p.muted, marginTop: '5px', display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {data.basics.phone && data.basics.email && <span>|</span>}
      ...
      <div style={{ display: 'flex', padding: '0 36px 28px' }}>
        <div style={{ flex: '0 0 63%', paddingRight: '20px' }}>
        ...
        <div style={{ flex: '0 0 37%', paddingLeft: '20px', borderLeft: `1px solid ${p.divider}` }}>
